...
exports.validateOllamaSetup = validateOllamaSetup;
 ...
 */
function loadOllamaConfig(config) {
    return {
 ...
 */
async function checkOllamaBinary() {
    try {
        await execAsync('which ollama', { timeout: 5000 });
    }
    catch (error) {
        throw new Error('Ollama binary not found. Please install Ollama from https://ollama.ai or ensure it is in your PATH.');
    }
}
/**
...
async function checkModelAvailability(model) {
    try {
        ...
        if (!stdout.includes(model)) {
            throw new Error(`Model "${model}" is not available. Please run "ollama pull ${model}" to download it, or choose a different model.`);
        }
    }
    catch (error) {
        ...
        throw new Error(`Failed to check model availability: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
}
/**
 ...
 */
function createDocumentationPrompt(sourceCode, maxLength) {
...
function parseOllamaResponse(response) {
    try {
        ...
        throw new Error(`Failed to parse Ollama response: ${error instanceof Error ? error.message : 'Unknown parsing error'}`);
    }
}
/**
 * Sends a prompt to Ollama and returns the raw response
 */
async function sendPromptToOllama(prompt, model, timeout) {
    try {
 ...
 */
async function generateDocumentation(sourceCode, config) {
    ...
    const response = await sendPromptToOllama(prompt, ollamaConfig.model, ollamaConfig.timeout);
 ...
 */
async function validateOllamaSetup(model) {
    try {
